...
async def get_stations(db: DatabaseDep) -> list[StationSchema]:
    return await crud_stations.get_stations(db)


@router.get("/stations/{station_id}", response_model=StationSchema)
async def get_station(station_id: UUID, db: DatabaseDep) -> StationSchema:
    return await crud_stations.get_station(db, station_id)


@router.post("/stations", response_model=StationSchema)
async def post_station(
    station_create: StationCreateSchema, db: DatabaseDep
) -> StationSchema:
    return await crud_stations.create_station(db, station_create)
...
async def patch_station(
    ...
    return await crud_stations.update_station(db, station_id, station_update)


@router.delete("/stations/{station_id}", response_model=StationSchema)
async def delete_station(station_id: UUID, db: DatabaseDep) -> StationSchema:
    return await crud_stations.delete_station(db, station_id)
...
async def post_transformer(
    ...
    return await crud_transformers.create_transformer(db, transformer_db_create)


@router.get(
    "/station/{station_id}/measurements", response_model=list[MeasurementSchema]
)
async def get_measurements_by_station_id(
    station_id: UUID,
    db: DatabaseDep,
    ...
    query = select(MeasurementModel).where(MeasurementModel.station_id == station_id)
    ...
    query = query.order_by(desc(MeasurementModel.measured_at)).limit(limit).offset(skip)
